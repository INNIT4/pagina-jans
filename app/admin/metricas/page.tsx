...
export default function MetricasPage() {
  ...
  const clientesCurr   = useMemo(() =>
    new Set(pagadosPeriod.filter((b) => b.celular).map((b) => b.celular)).size
  , [pagadosPeriod]);
  const clientesPrevVal = useMemo(() =>
    new Set(pagadosPrev.filter((b) => b.celular).map((b) => b.celular)).size
  , [pagadosPrev]);
  ...
  const riesgoVencer = useMemo(() => {
    ...
    return pendientesAll.filter((b) => {
      const d = b.created_at?.toDate?.();
      return d && d < threshold;
    });
  }, [pendientesAll]);
  const riesgoVencerValor = useMemo(
    () => riesgoVencer.reduce((s, b) => s + b.precio_total, 0),
    [riesgoVencer]
  );
  ...
  const codesEnRiesgo = useMemo(
    () => codes.filter((c) => c.activo && c.max_usos > 0 && c.usos >= c.max_usos - 1),
    [codes]
  );

  const rifasSinGanador = useMemo(
    () => rifas.filter((r) => !r.activa && !r.ganador),
    [rifas]
  );
  ...
  const byEstado = useMemo(() => {
    ...
    return Array.from(map.entries())
      .map(([estado, data]) => ({
        ...
        rpc: data.count > 0 ? data.revenue / data.count : 0,
      }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 10);
  }, [boletos]);
  ...
  const diasSemana = useMemo(() => {
    ...
    const map = new Array(7).fill(0) as number[];
    boletosPeriod.forEach((b) => {
      const d = b.created_at?.toDate?.();
      if (d) map[d.getDay()]++;
    });
    return map.map((count, i) => ({ dia: nombres[i], count }));
  }, [boletosPeriod]);
  ...
  const clientesRecurrentes = useMemo(() => {
    ...
    pagadosAll.forEach((b) => {
      ...
      const existing = map.get(key);
      if (existing) {
        existing.rifasSet.add(b.rifa_id);
        existing.total += b.precio_total;
      } else {
        map.set(key, { nombre: `${b.nombre} ${b.apellidos}`.trim(), rifasSet: new Set([b.rifa_id]), total: b.precio_total });
      }
    });
    const all = Array.from(map.values());
    const recurrentes = all
      .filter((c) => c.rifasSet.size >= 2)
      .sort((a, b) => b.total - a.total)
      .map((c) => ({ nombre: c.nombre, rifas: c.rifasSet.size, total: c.total }));
  ...
  return (
    <div className="space-y-8">
              ...
              <div className="flex items-start gap-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-bold text-red-700 dark:text-red-400">
                    {riesgoVencer.length} boleto{riesgoVencer.length !== 1 ? "s" : ""} en riesgo de vencer
                  </p>
                  <p className="text-xs text-red-600 dark:text-red-500 mt-0.5">
                    Pendientes hace más de 20h · Valor: {currency(Math.round(riesgoVencerValor))}
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-bold text-yellow-700 dark:text-yellow-400">
                    {rifasOcupacionBaja.length} rifa{rifasOcupacionBaja.length !== 1 ? "s" : ""} con ocupación &lt; 15% y sorteo en &lt; 14 días
                  </p>
                  <p className="text-xs text-yellow-600 dark:text-yellow-500 mt-0.5">
                    {rifasOcupacionBaja.map((r) => r.nombre).join(", ")}
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-bold text-orange-700 dark:text-orange-400">
                    {codesEnRiesgo.length} código{codesEnRiesgo.length !== 1 ? "s" : ""} a punto de agotarse
                  </p>
                  <p className="text-xs text-orange-600 dark:text-orange-500 mt-0.5">
                    {codesEnRiesgo.map((c) => `${c.codigo} (${c.usos}/${c.max_usos})`).join(", ")}
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-bold text-blue-700 dark:text-blue-400">
                    {rifasSinGanador.length} rifa{rifasSinGanador.length !== 1 ? "s" : ""} finalizada{rifasSinGanador.length !== 1 ? "s" : ""} sin ganador registrado
                  </p>
                  <p className="text-xs text-blue-600 dark:text-blue-500 mt-0.5">
                    {rifasSinGanador.map((r) => r.nombre).join(", ")}
                  </p>
                </div>
              </div>
            )}
      ...
      {/* ── Tendencia de boletos pagados (period) ── */}
      {period > 0 && trendPagados !== null && (
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl px-5 py-3 border border-slate-100 dark:border-slate-700 flex flex-wrap gap-6 items-center">
          ...
          <p className="text-xs text-slate-400">
            {pagadosPeriod.length} pagados este período · {pagadosPrev.length} en el anterior
          </p>
            ...
            {[
              ...
              <div key={s.label}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className={`font-black ${s.textColor}`}>
                    {s.count} <span className="font-normal text-slate-400">({pct(s.count, boletos.length)}%)</span>
                  </span>
                </div>
        ...
        <div className="flex items-end gap-1 h-36">
          {timeSeries.map((d) => (
            <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group relative">
              ...
              <div
                className="w-full rounded-t-lg bg-red-500 dark:bg-red-600 transition-all duration-700"
      ...
      {/* ── Velocidad de ventas y proyección ── */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow border border-slate-100 dark:border-slate-700 p-6">
      ...
      {/* ── Top clientes + Distribución por estado (enhanced) ── */}
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow border border-slate-100 dark:border-slate-700 p-6">
        ...
        <div className="space-y-3">
          {distribucionTamano.map((bk) => (
            <HBar
              ...
              label={bk.label}
              value={bk.count}
              max={Math.max(...distribucionTamano.map((x) => x.count), 1)}
              formatted={`${bk.count} boleto${bk.count !== 1 ? "s" : ""}`}
              color="bg-indigo-500"
            />
          ))}
        </div>
        <p className="text-xs text-slate-400 mt-3">
          Basado en {pagadosAll.length} boleto{pagadosAll.length !== 1 ? "s" : ""} pagado{pagadosAll.length !== 1 ? "s" : ""} (todo el tiempo)
        </p>
      </div>
              ...
              <div className="space-y-2">
                {horasPico.map((h) => (
                  <HBar
                    ...
                    value={h.count}
                    max={horasPico[0].count}
                    formatted={`${h.count} boleto${h.count !== 1 ? "s" : ""}`}
                    color="bg-teal-500"
                  />
                ))}
              </div>
          ...
          <div className="space-y-2">
            {diasSemana.map((d) => (
              <HBar
                ...
                label={d.dia}
                value={d.count}
                max={Math.max(...diasSemana.map((x) => x.count), 1)}
                formatted={`${d.count} boleto${d.count !== 1 ? "s" : ""}`}
                color="bg-violet-500"
              />
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-5">
          ...
          <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 text-center">
            <p className="text-2xl font-black text-slate-600 dark:text-slate-300">
              {clientesRecurrentes.totalClientes - clientesRecurrentes.recurrentes.length}
            </p>
      ...
      {/* ── Números más populares + Códigos de descuento ── */}
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow border border-slate-100 dark:border-slate-700 p-6">
