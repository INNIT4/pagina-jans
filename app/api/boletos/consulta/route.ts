...
export async function GET(req: NextRequest) {
  try {
    ...
    const { success } = await getRatelimit().limit(`consulta:${ip}`);
    if (!success)
    ...
    if (folio) {
      ...
      snap = await db.collection("boletos").where("celular", "==", celular).limit(50).get();
    } else if (numero) {
    ...
    console.error("[boletos/consulta]", err);
